...
def validate_stage_config(data: Any) -> dict:
    ...
    for stage in STAGES:
        ...
        require(isinstance(spec.get("name"), str) and spec["name"], f"{stage}.name 缺失")
        for field in ("requires", "reviews", "checks"):
            require(isinstance(spec.get(field, []), list), f"{stage}.{field} 必须是数组")
        for node_id in spec.get("requires", []):
            require(isinstance(node_id, str) and ID_RE.match(node_id) is not None,
...
def validate_evidence_graph(data: Any) -> dict:
    ...
    for node_id, node in nodes.items():
        ...
        require(node.get("status") in EVIDENCE_STATUSES, f"节点 status 非法: {node_id}")
